...
/** =がクリックされたときの処理 */
function dspResult() {
  //初回入力以外＆前回数字の入力のときに処理を実行
  if(wkFirst === "0" && wkBefore === "0") {
    elementcalcLog.innerHTML = elementcalcLog.innerHTML + Number(elementResult.innerHTML);//calculate log
    calculator();
    wkCalc = "="; //equal save
    wkBefore = "1";
  }
...
function clear() {
  elementResult.innerHTML = 0; //clear
  elementcalcLog.innerHTML = ""; //clear
  wkFirst = "1";
  wkTotal = 0;
  wkCalc = "+";
  wkBefore = "1";
