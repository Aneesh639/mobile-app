...
function delLocalStorage(){
    ...
    del.addEventListener("click",
        function(e){
            ...
            let w_cnt=0; //if selected one make to 1  version-up3 change let w_sel="0";==> w_cnt=0
            w_cnt = selectCheckBox("del"); //version-up3 change  function() ==>(del)

            if(w_cnt>=1){  //version-up3 change w_sel ==> w_cnt
                let w_confirm = window.confirm("LocalStorage から選択されている" + w_cnt + "件を削除(delete)しますか？");
                if(w_confirm === true){
                    for(let i=0; i < chkbox1.length; i++){//version-up3 add
                        if(chkbox1[i].checked){
                            localStorage.removeItem(table1.rows[i+1].cells[1].firstChild.data);//version-up3 change
                        }
                    }  
                    viewStorage();
...
function selectCheckBox(mode){ //version up 3 function mode
